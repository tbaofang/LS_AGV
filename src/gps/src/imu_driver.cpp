...
void dataParse(vector<unsigned char>& data)
{
	...
	float roll = byte2Float(begin+28, begin+32);
	float pitch = byte2Float(begin+32, begin+36);
	float yaw = byte2Float(begin+36, begin+40);
...
void dataProcess(vector<unsigned char>& data)
{
	...
	if(distance(position1, position2) == 42)
	{
